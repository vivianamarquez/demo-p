...
def create_prolific_study():
    ...
    # Parse age range
    age_range = study_config.get('age_range', '18-25')
    age_min, age_max = map(int, age_range.split('-'))
    
    # Get countries
    countries = study_config.get('countries', ['US'])
    
    # Map UI country codes to Prolific ChoiceIDs for 'current-country-of-residence'
    ...
    # Add current-country-of-residence filter if countries are specified
